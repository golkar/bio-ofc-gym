...
class FlyLinEnv(gym.Env, EzPickle):
    metadata = {
    ...
    def step(self, action):
        ...
        # Engines
        dispersion = [self.noise * self.np_random.uniform(-1.0, +1.0) / SCALE for _ in range(2)]
        ...
        if (self.continuous) or (not self.continuous and action == 2):
            ...
            if self.continuous:
                ...
                m_power = 1.0
            self.lander.ApplyLinearImpulse((2 * dispersion[0] * MAIN_ENGINE_POWER , (4/SCALE * m_power + 2 * dispersion[1]) * MAIN_ENGINE_POWER),
...
class FlyLinEnvContinuous(FlyLinEnv):
    continuous = True
