    ...
    def step(self, action):
        ...
        if (self.continuous) or (not self.continuous and action == 2):
            ...
            if self.continuous:
                ...
                m_power = 1.0
            try: self.lander.ApplyLinearImpulse((2 * dispersion[0] * MAIN_ENGINE_POWER , (4/SCALE * m_power + 2 * dispersion[1]) * MAIN_ENGINE_POWER),
                                           self.lander.position,
                                           True)
            except: print((2 * dispersion[0] * MAIN_ENGINE_POWER , (4/SCALE * m_power + 2 * dispersion[1]) * MAIN_ENGINE_POWER),self.lander.position)
        
        self.lander.ApplyForce((self.wind,0),(self.lander.position[0], self.lander.position[1]), True)
        self.wind = np.clip(self.wind + self.wind_scale*WIND_STEP * (np.random.randint(3)-1),-self.wind_scale*WIND_MAX,self.wind_scale*WIND_MAX)
        ...
        if (self.continuous) or (not self.continuous and action in [1, 3]):
            ...
            if self.continuous:
                ...
                direction = action-2
                s_power = 1.0
            self.lander.ApplyLinearImpulse((direction * SIDE_ENGINE_POWER * s_power, 0),
                                           self.lander.position,
                                           True)
...
#         self.lander.angle=0
    ...
    def render(self, mode='human', text=None):
        ...
        self.viewer.draw_polygon([(8.5*VIEWPORT_W/SCALE/10, 1.5*VIEWPORT_H/SCALE/10), 
                                 ((17+3*self.wind/WIND_MAX/self.wind_scale)*VIEWPORT_W/SCALE/20, 1.25*VIEWPORT_H/SCALE/10),
...
class FlyLinDelayedEnvContinuous(FlyLinDelayedEnv):
    continuous = True
